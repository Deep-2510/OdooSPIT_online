...
const TransferForm = ({ transfer, onSubmit, onCancel, loading = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.sourceWarehouseId === formData.destinationWarehouseId) {
      addNotification('Source and destination warehouses cannot be the same', 'error')
      return
    }
    ...
    try {
      ...
      addNotification(error.message || 'Failed to create transfer', 'error')
    }
  }
    ...
    <div className="bg-white rounded-lg shadow-sm border">
      <div className="px-6 py-4 border-b">
        <h2 className="text-xl font-semibold text-gray-900">
          {transfer ? 'Edit Transfer' : 'Create Internal Transfer'}
        </h2>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, destinationWarehouseId: e.target.value }))}
              required
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
